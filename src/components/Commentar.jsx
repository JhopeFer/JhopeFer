...
const getSupabaseErrorMessage = (error) => {
    ...
    if (error.message?.includes('secret API key')) {
        return 'Comments are unavailable: the site is using a Supabase secret key in the browser. Replace VITE_SUPABASE_ANON_KEY with the anon/public key from your Supabase dashboard.';
    }
    ...
    if (error.code === '42P01') {
        return 'Comments table not found. Create the portfolio_comments table in Supabase.';
    }

    return error.message || 'Failed to connect to comments. Please try again.';
};
...
const Comment = memo(({ comment, formatDate, index, isPinned = false }) => (
    <div 
        className={`px-4 pt-4 pb-2 rounded-xl border transition-all group hover:shadow-lg hover:-translate-y-0.5 ${
            isPinned 
                ? 'bg-gradient-to-r from-red-600/10 to-red-500/10 border-red-600/30 hover:bg-gradient-to-r hover:from-red-600/15 hover:to-red-500/15' 
    ...
    >
        ...
        <div className="flex items-start gap-3">
            {comment.profile_image ? (
                ...
                <div className={`p-2 rounded-full text-red-400 group-hover:bg-red-600/30 transition-colors ${
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
                ...
                className="relative w-full h-12 bg-gradient-to-r from-[#8B1C1C] to-[#DC143C] rounded-xl font-medium text-white overflow-hidden group transition-all duration-300 hover:scale-[1.02] hover:shadow-lg active:scale-[0.98] disabled:opacity-50 disabled:hover:scale-100 disabled:cursor-not-allowed"
            >
